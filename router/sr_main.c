...
int main(int argc, char **argv)
{
    ...
    // New nat stuff
    int natFlag = 0;
    int IQT = DEFAULT_ICMP_QUERY_TIMEOUT;
    int TET = DEFAULT_TCP_ESTABLISHED_TIMEOUT;
    int TTT = DEFAULT_TCP_TRANSITORY_TIMEOUT;
    ...
    while ((c = getopt(argc, argv, "hs:v:p:u:t:r:l:T:nI:E:R:")) != EOF)
    {
        switch (c)
        {
            ...
            // I Integer - ICMP Query timeoutinterval
            case 'I':
                IQT = atoi((char *) optarg);
                break;
            case 'E':
                TET = atoi((char *) optarg);
                break;
            case 'R':
                TTT = atoi((char *) optarg);
                break;
        } /* switch */
    } /* -- while -- */
