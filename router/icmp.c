...
void processICMP(struct sr_instance *sr, uint8_t *packet, unsigned int len){
    ...
    if (len < sizeof(sr_ethernet_hdr_t) + (ipHeader->ip_hl * 4) + sizeof(sr_icmp_hdr_t)) {
        printf("ICMP Header insufficient length... Terminating.\n");
        return;
    }

	// Construct the ICMP Header from the packet
    sr_icmp_hdr_t *icmpHeader = (sr_icmp_hdr_t *) (packet + sizeof(sr_ethernet_hdr_t) + (ipHeader->ip_hl * 4));
...
void sendICMP(struct sr_instance *sr, uint8_t *packet, unsigned int len, uint8_t type, uint8_t code){
	...
	/* Case if ICMP TYPE is a Unreachable: Type 3 ICMP */
	else if (type == DESTINATION_UNREACHABLE) {

		// Calculate the new length of the packet to be sent (convert to Type 3 ICMP)
		unsigned int newLength = sizeof(sr_ethernet_hdr_t) + sizeof(sr_ip_hdr_t) + sizeof(sr_icmp_t3_hdr_t);
		...
		sr_icmp_t3_hdr_t *type3Header = (sr_icmp_t3_hdr_t *) (newPacket + sizeof(sr_ethernet_hdr_t) + sizeof(sr_ip_hdr_t));
		...
		newIpHeader->ip_v = 4;
		newIpHeader->ip_hl = sizeof(sr_ip_hdr_t) / 4;
		newIpHeader->ip_tos = 0;
		newIpHeader->ip_len = htons(sizeof(sr_ip_hdr_t) + sizeof(sr_icmp_t3_hdr_t));
		newIpHeader->ip_id = htons(0);
		newIpHeader->ip_off = htons(IP_DF);
		newIpHeader->ip_ttl = 64;
		newIpHeader->ip_p = ip_protocol_icmp;
		// Check for codes here: Port unreachable goes back to sender
		if (code == PORT_UNREACHABLE_CODE) {
		...
		type3Header->icmp_type = type;
		type3Header->icmp_code = code;
		...
		type3Header->icmp_sum = cksum(type3Header, sizeof(sr_icmp_t3_hdr_t));
	...
	/* Case if ICMP Type is Time exceeded: TYPE 11 ICMP*/
		...
		unsigned int newLength = sizeof(sr_ethernet_hdr_t) + sizeof(sr_ip_hdr_t) + sizeof(sr_icmp_t11_hdr_t);
		uint8_t *newPacket = (uint8_t *)malloc(newLength);
		assert(newPacket);

		sr_ethernet_hdr_t *newEthernetHeader = (sr_ethernet_hdr_t *)newPacket;
		sr_ip_hdr_t *newIpHeader = (sr_ip_hdr_t *)(newPacket + sizeof(sr_ethernet_hdr_t));
		sr_icmp_t11_hdr_t *type11Header = (sr_icmp_t11_hdr_t *)(newPacket + sizeof(sr_ethernet_hdr_t) + sizeof(sr_ip_hdr_t));
		...
		newIpHeader->ip_hl = sizeof(sr_ip_hdr_t) / 4;
		newIpHeader->ip_tos = 0;
		newIpHeader->ip_len = htons(sizeof(sr_ip_hdr_t) + sizeof(sr_icmp_t11_hdr_t));
		newIpHeader->ip_id = htons(0);
		newIpHeader->ip_off = htons(IP_DF);
		newIpHeader->ip_ttl = 64;
		newIpHeader->ip_p = ip_protocol_icmp;
		...
		type11Header->icmp_sum = cksum(type11Header, sizeof(sr_icmp_t11_hdr_t));
