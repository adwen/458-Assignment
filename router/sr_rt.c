...
struct sr_rt *findLPM(struct sr_instance *sr, uint32_t ip){
    ...
    // While the pointer isnt pointing to null
    while (tablePointer) {

        // Check for the LPM
        if ((tablePointer->dest.s_addr & tablePointer->mask.s_addr) == (ip & tablePointer->mask.s_addr)) {

            // If we have a match, record if the length is the longest
            if (foundLength <= tablePointer->mask.s_addr) {
                LPM = tablePointer;
                foundLength = tablePointer->mask.s_addr;
            }
        }

        // Traverse to the next routing table entry
